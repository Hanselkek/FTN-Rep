...
Background_1 = pygame.image.load(bg_info).convert_alpha()
...
mixer.music.play(-1)
# Controls
ControlsText = NoobsFramework.showKeys().create("Controls: ", 19)

Up_ArrowT = NoobsFramework.showKeys().create("UP - Up Arrow ", 16)
Down_ArrowT = NoobsFramework.showKeys().create("DOWN - DOWN Arrow ", 16)

Left_ArrowT = NoobsFramework.showKeys().create("Left - Left Arrow ", 16)
Right_ArrowT = NoobsFramework.showKeys().create("Right - Right Arrow ", 16)
# Noobs Variables
...
def addNoob_Sleepy(x, y):
    ...
    if sleepy_HAS_BEEN_TAKEN == False:
        if isCollided(default_PosX, default_PosY, x, y) < 27:
            ...
            NoobsFramework.update().update_ui(1, "Sleepy Noob")
...
while Game_Is_Open:
    ...
    screen.blit(NoobsFramework.show().show_ui(), (27, 55))
    screen.blit(NoobsFramework.showRec().showRecent(), (27, 94))

    NoobsFramework.showKeys().option(27, 315, ControlsText, screen)

    NoobsFramework.showKeys().option(27, 335, Up_ArrowT, screen)
    NoobsFramework.showKeys().option(27, 355, Down_ArrowT, screen)

    NoobsFramework.showKeys().option(27, 375, Left_ArrowT, screen)
    NoobsFramework.showKeys().option(27, 395, Right_ArrowT, screen)
